...
#| tbl-cap: test
#| results: asis
models_wb_disc <- map(scholars_imp, function(x) {
  lmer(formula_disc, data=x)
})
models_wb_controls <- map(scholars_imp, function(x) {
  lmer(formula_controls, data=x)
})

knitreg(lapply(list(models_wb_disc, models_wb_controls), pool),
        custom.coef.names = coef_labels,
        digits=2,
        custom.model.names = c("W-B Model","w/controls"),
...
* Between disciplines, a 1% increase in the mean percent of sole authored publications is associated witha a 0.47 reduction in the mean h-index. 
...
models_wb_nlinear <- map(scholars_imp, function(x) {
  lmer(formula_nlinear, data=x)
})

mean(sapply(models_wb_nlinear, BIC))
mean(sapply(models_wb_controls, BIC))


knitreg(lapply(list(models_wb_controls, models_wb_nlinear), pool), digits=3,
