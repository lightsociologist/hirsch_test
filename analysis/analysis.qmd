...
#| tbl-cap: test
#| results: asis
models_wb_disc_imp <- map(scholars_imp, function(x) {
  lmer(formula_disc, data=x)
})
models_wb_controls_imp <- map(scholars_imp, function(x) {
  lmer(formula_controls, data=x)
})

knitreg(lapply(list(models_wb_disc_imp, models_wb_controls_imp), pool),
...
# now lets do the same in the imputed data
