...
formula_disc <- h_index ~ I(prop_female-disc_prop_female)+
...
There are a few missing fields here and I don't know if I like putting Business in Social Sciences. We could break Economics out of here and put business by itself.
...
models_big_field <- scholars %>%
  group_by(big_field) %>%
  group_split() %>%
  map(~ lmer(formula_controls, data=.x))

knitreg(models_big_field, 
        custom.coef.names = coef_labels,
