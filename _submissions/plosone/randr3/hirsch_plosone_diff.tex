...
%DIF DEL ../randr2/hirsch_plosone_randr2.tex   Wed Aug 21 11:48:37 2024
%DIF ADD hirsch_plosone_randr3.tex             Thu Oct 17 12:33:09 2024
...
%DIF < x\textpilcrow Membership list can be found in the Acknowledgments section.
...
\DIFdelbegin %DIFDELCMD < {\Large
%DIFDELCMD < \textbf\newline{Inequality in measuring scholarly success: Variation in
%DIFDELCMD < the \emph{h}-index within and between
%DIFDELCMD < disciplines} %%%
...
penalties within fields. \DIFdelbegin \DIFdel{Conclusions include }\DIFdelend \DIFaddbegin \DIFadd{In conclusion, we recommend }\DIFaddend continued caution
when using the \emph{h}-index or similar metrics for valuation purposes
\DIFaddbegin \DIFadd{and the prioritization of substantive valuations from disciplinary
experts}\DIFaddend .
...
continued efforts towards metrics literacy, or the ability \DIFaddbegin \DIFadd{to
}\DIFaddend ``critically assess and effectively and ethically use scholarly
...
\DIFaddbegin \DIFadd{Academics use the }\emph{\DIFadd{h}}\DIFadd{-index informally to assess impact via popular
databases like Google Scholar, but also formally as listed in promotion
policies and faculty handbooks \mbox{%DIFAUXCMD
\citep{merga_should_2020}}\hskip0pt%DIFAUXCMD
. }\DIFaddend The
...
\DIFdelbegin \DIFdel{From well-known pipeline effects to publication differences to varying
effects of parenthood,
research has shown }\DIFdelend \DIFaddbegin \DIFadd{In this article, we examine within and between discipline variation in
the }\emph{\DIFadd{h}}\DIFadd{-index and its relationship to inequality and disciplinary
culture at both levels. We focus our attention on gender inequality,
both because prior research has demonstrated }\DIFaddend the multifaceted ways \DIFdelbegin \DIFdel{some
...
.
Differences exist in the distribution of resources to disciplines
, such
as the prioritization of the natural sciences and engineering in federal
funding and the unequal distribution of resources, like salaries, to departments, the proxies for disciplines on
university campuses
\mbox{%DIFAUXCMD
\citep{katz_metrics_2020}}\hskip0pt%DIFAUXCMD
. At the same time, numerous scholars have
drawn critical attention to the increasing push to quantify aspects of
social life \mbox{%DIFAUXCMD
\citep{mennicken_what_2019}}\hskip0pt%DIFAUXCMD
. In the case of scholarly
...
in less productive and
innovative scholarship
\mbox{%DIFAUXCMD
\citep{edwards_academic_2017, muller_tyranny_2019} }\hskip0pt%DIFAUXCMD
and efforts to ``game
metrics'' at either the journal or individual-level
\mbox{%DIFAUXCMD
\citep{siler_who_2022}}\hskip0pt%DIFAUXCMD
. Examining within and between discipline
variation in the }\emph{\DIFdel{h}}%DIFAUXCMD
\DIFdel{-index offers insight into how metrics relate
...
of simplifying summaries of academic labor through quantification. Here}\DIFdelend \DIFaddbegin \DIFadd{that
...
fields and can affect overall citations metrics. Specifically}\DIFaddend , we ask
...
\item
  ...
  \emph{h}-index?
\item
  How do \DIFdelbegin \DIFdel{social factors, specifically }\DIFdelend gender and sole authorship \DIFdelbegin \DIFdel{,
  }\DIFdelend contribute to this variation \DIFaddbegin \DIFadd{at both
  levels}\DIFaddend ?
\item
  How do \DIFaddbegin \DIFadd{these }\DIFaddend within and between discipline factors vary across fields
  (e.g., social sciences, medical sciences, natural sciences)?
\end{enumerate}

\DIFaddbegin \DIFadd{Examining within and between discipline variation in the }\emph{\DIFadd{h}}\DIFadd{-index
...
labor through quantification. }\DIFaddend While prior work examines the
...
trajectories dependent on \DIFdelbegin \DIFdel{gender and sole
authorship}\DIFdelend \DIFaddbegin \DIFadd{these same factors}\DIFaddend .

\subsection{\DIFaddbegin \DIFadd{Gender }\DIFaddend Inequality in
Science}\DIFdelbegin %DIFDELCMD < \label{inequality-in-science}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{gender-inequality-in-science}
\DIFaddend 
...
we know'' \DIFdelbegin \DIFdel{(\mbox{%DIFAUXCMD
\citep{cetina_epistemic_1999}}\hskip0pt%DIFAUXCMD
, pg. 1). }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[pg. 1]{cetina_epistemic_1999}}\hskip0pt%DIFAUXCMD
. }\DIFaddend The questions that
...
\DIFaddbegin \DIFadd{Together, prior research on inequality points to processes that may
impact within and between discipline variation in the }\emph{\DIFadd{h}}\DIFadd{-index.
The }\emph{\DIFadd{h}}\DIFadd{-index plays a role in academic hierarchies through informal
...
\DIFaddend \subsection{Inequality Between Disciplines and
...
fraudulent work \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{edwards_academic_2017}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{and efforts to ``game'' the system at either the journal
or individual-level \mbox{%DIFAUXCMD
\citep{edwards_academic_2017, siler_who_2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend , while
simultaneously resulting in mental health trauma for academic workers
experiencing hypercompetitive markets and suspicious management
...
%DIFDELCMD < \hline
%DIFDELCMD <  & Model 1 & Model 2 & Model 3 & Model 4 \\
%DIFDELCMD < \hline
%DIFDELCMD < Intercept                               & $39.32^{*}$ & $69.64^{*}$  & $68.86^{*}$  & $68.91^{*}$  \\
%DIFDELCMD <                                         & $(1.80)$    & $(2.12)$     & $(2.13)$     & $(2.06)$     \\
%DIFDELCMD < Prob. [0-1] of female name$\dagger$     & $-3.00^{*}$ &              & $-3.67^{*}$  & $-2.63^{*}$  \\
%DIFDELCMD <                                         & $(0.23)$    &              & $(0.22)$     & $(0.21)$     \\
%DIFDELCMD < Disc. mean percent female name          & $-0.15^{*}$ &              & $0.10^{*}$   & $0.11^{*}$   \\
%DIFDELCMD <                                         & $(0.07)$    &              & $(0.05)$     & $(0.05)$     \\
%DIFDELCMD < Percent sole authored pubs$\dagger$     &             & $-0.41^{*}$  & $-0.42^{*}$  & $-0.47^{*}$  \\
%DIFDELCMD <                                         &             & $(0.01)$     & $(0.01)$     & $(0.01)$     \\
%DIFDELCMD < Disc. mean percent sole authored (log)  &             & $-12.87^{*}$ & $-13.39^{*}$ & $-13.53^{*}$ \\
%DIFDELCMD <                                         &             & $(0.79)$     & $(0.82)$     & $(0.79)$     \\
%DIFDELCMD < Career length*                          &             &              &              & $0.35^{*}$   \\
%DIFDELCMD <                                         &             &              &              & $(0.01)$     \\
%DIFDELCMD < Specialization [0-1]*                   &             &              &              & $-2.66^{*}$  \\
%DIFDELCMD <                                         &             &              &              & $(0.44)$     \\
%DIFDELCMD < Highly productive uni. count (stdized)* &             &              &              & $1.72^{*}$   \\
%DIFDELCMD <                                         &             &              &              & $(0.07)$     \\
%DIFDELCMD < \hline
%DIFDELCMD < Residual SD between discipline          & $11.86$     & $7.41$       & $7.33$       & $7.05$       \\
%DIFDELCMD < Residual SD within discipline           & $17.41$     & $16.83$      & $16.78$      & $16.34$      \\
%DIFDELCMD < N (discipline)                          & $174$       & $174$        & $174$        & $174$        \\
%DIFDELCMD < N (individual)                          & $54825$     & $54825$      & $54825$      & $54825$      \\
%DIFDELCMD < \hline
...
%DIFDELCMD < \hline
%DIFDELCMD <  & Humanities & Medical & Prof. & Soc. Sci. & STEM \\
%DIFDELCMD < \hline
%DIFDELCMD < Intercept                               & $79.45^{*}$  & $93.04^{*}$  & $47.21^{*}$ & $77.81^{*}$  & $58.73^{*}$  \\
%DIFDELCMD <                                         & $(12.68)$    & $(6.37)$     & $(8.23)$    & $(4.57)$     & $(5.18)$     \\
%DIFDELCMD < Prob. [0-1] of female name$\dagger$     & $-1.30^{*}$  & $-3.73^{*}$  & $-1.57^{*}$ & $-2.11^{*}$  & $-1.25^{*}$  \\
%DIFDELCMD <                                         & $(0.50)$     & $(0.31)$     & $(0.76)$    & $(0.43)$     & $(0.34)$     \\
%DIFDELCMD < Disc. mean percent female name          & $-0.02$      & $0.07$       & $-0.01$     & $-0.13^{*}$  & $0.12$       \\
%DIFDELCMD <                                         & $(0.09)$     & $(0.08)$     & $(0.21)$    & $(0.05)$     & $(0.17)$     \\
%DIFDELCMD < Percent sole authored pubs$\dagger$     & $-0.16^{*}$  & $-0.66^{*}$  & $-0.26^{*}$ & $-0.24^{*}$  & $-0.49^{*}$  \\
%DIFDELCMD <                                         & $(0.01)$     & $(0.01)$     & $(0.02)$    & $(0.01)$     & $(0.01)$     \\
%DIFDELCMD < Disc. mean percent sole authored (log)  & $-15.38^{*}$ & $-21.79^{*}$ & $-6.84^{*}$ & $-13.66^{*}$ & $-10.57^{*}$ \\
%DIFDELCMD <                                         & $(3.08)$     & $(3.02)$     & $(3.34)$    & $(1.19)$     & $(1.83)$     \\
%DIFDELCMD < Career length*                          & $0.13^{*}$   & $0.45^{*}$   & $0.29^{*}$  & $0.31^{*}$   & $0.30^{*}$   \\
%DIFDELCMD <                                         & $(0.02)$     & $(0.01)$     & $(0.03)$    & $(0.02)$     & $(0.01)$     \\
%DIFDELCMD < Specialization [0-1]*                   & $-3.13^{*}$  & $0.03$       & $-0.46$     & $-6.31^{*}$  & $-4.36^{*}$  \\
%DIFDELCMD <                                         & $(1.10)$     & $(0.79)$     & $(1.55)$    & $(1.03)$     & $(0.57)$     \\
%DIFDELCMD < Highly productive uni. count (stdized)* & $0.55^{*}$   & $1.51^{*}$   & $0.60^{*}$  & $0.38^{*}$   & $2.16^{*}$   \\
%DIFDELCMD <                                         & $(0.21)$     & $(0.12)$     & $(0.29)$    & $(0.18)$     & $(0.10)$     \\
%DIFDELCMD < \hline
%DIFDELCMD < Residual SD between discipline          & $4.51$       & $6.70$       & $4.27$      & $3.13$       & $6.22$       \\
%DIFDELCMD < Residual SD within discipline           & $5.98$       & $18.69$      & $9.48$      & $11.31$      & $14.41$      \\
%DIFDELCMD < N (discipline)                          & $15$         & $52$         & $12$        & $28$         & $67$         \\
%DIFDELCMD < N (individual)                          & $856$        & $25581$      & $1228$      & $3987$       & $23173$      \\
%DIFDELCMD < \hline
...
\DIFdelbegin \DIFdel{Data limitations suggest several }\DIFdelend \DIFaddbegin \DIFadd{Several limitations suggest }\DIFaddend avenues for future research. First, the data
...
like race\DIFdelbegin \DIFdel{and class, }\DIFdelend \DIFaddbegin \DIFadd{, class, ethnicity, sexual orientation, and whether one is a
foreign-born academic or not }\DIFaddend remain under-studied. Future research would
benefit from data linking \DIFdelbegin \DIFdel{self-reported race and class }\DIFdelend \DIFaddbegin \DIFadd{self-reports of these characteristics }\DIFaddend to large
...
evaluation in academia should be used with caution \DIFaddbegin \DIFadd{at minimum }\DIFaddend and
...
proposes to facilitate. \DIFaddbegin \DIFadd{Equitable evaluation should prioritize
substantive assessment by disciplinary experts and/or content
specialists. }\DIFaddend Like other forms of quantification, academic metrics
...
\DIFaddbegin \bibitem{merga_should_2020}
\DIFadd{Merga MK, Mat~Roni S, Mason S.
}\newblock \DIFadd{Should }{{\DIFadd{Google Scholar}}} \DIFadd{Be Used for Benchmarking against the Professoriate in Education?
}\newblock \DIFadd{Scientometrics. 2020;125(3):2505--2522.
}\newblock \DIFadd{doi:}{\DIFadd{10.1007/s11192-020-03691-3}}\DIFadd{.
}

\DIFaddend \bibitem{fox_gender_2005}
Fox MF.
...
\DIFdelbegin \bibitem{katz_metrics_2020}
\DIFdel{Katz Y, Matter U.
}%DIFDELCMD < \newblock %%%
\DIFdel{Metrics of }%DIFDELCMD < {{%%%
\DIFdel{Inequality}%DIFDELCMD < }}%%%
\DIFdel{: }%DIFDELCMD < {{%%%
\DIFdel{The Concentration}%DIFDELCMD < }} %%%
\DIFdel{of }%DIFDELCMD < {{%%%
\DIFdel{Resources}%DIFDELCMD < }} %%%
\DIFdel{in the }%DIFDELCMD < {{%%%
\DIFdel{U}%DIFDELCMD < }}%%%
\DIFdel{.}%DIFDELCMD < {{%%%
\DIFdel{S}%DIFDELCMD < }}%%%
\DIFdel{. }%DIFDELCMD < {{%%%
\DIFdel{Biomedical Elite}%DIFDELCMD < }}%%%
\DIFdel{.
}%DIFDELCMD < \newblock %%%
\DIFdel{Science as Culture. 2020;29(4):475--502.
}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1080/09505431.2019.1694882}%DIFDELCMD < }%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{mennicken_what_2019}
\DIFdel{Mennicken A, Espeland WN.
}%DIFDELCMD < \newblock %%%
\DIFdel{What's New with Numbers? }%DIFDELCMD < {{%%%
\DIFdel{Sociological}%DIFDELCMD < }} %%%
\DIFdel{Approaches to the Study of Quantification.
}%DIFDELCMD < \newblock %%%
\DIFdel{Annual Review of Sociology. 2019;45:223--245.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{edwards_academic_2017}
\DIFdel{Edwards MA, Roy S.
}%DIFDELCMD < \newblock %%%
\DIFdel{Academic }%DIFDELCMD < {{%%%
\DIFdel{Research}%DIFDELCMD < }} %%%
\DIFdel{in the 21st }%DIFDELCMD < {{%%%
\DIFdel{Century}%DIFDELCMD < }}%%%
\DIFdel{: }%DIFDELCMD < {{%%%
\DIFdel{Maintaining Scientific Integrity}%DIFDELCMD < }} %%%
\DIFdel{in a }%DIFDELCMD < {{%%%
\DIFdel{Climate}%DIFDELCMD < }} %%%
\DIFdel{of }%DIFDELCMD < {{%%%
\DIFdel{Perverse Incentives}%DIFDELCMD < }} %%%
\DIFdel{and }%DIFDELCMD < {{%%%
\DIFdel{Hypercompetition}%DIFDELCMD < }}%%%
\DIFdel{.
}%DIFDELCMD < \newblock %%%
\DIFdel{Environmental Engineering Science. 2017;34(1):51--61.
}%DIFDELCMD < \newblock %%%
\DIFdel{doi:}%DIFDELCMD < {%%%
\DIFdel{10.1089/ees.2016.0223}%DIFDELCMD < }%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{muller_tyranny_2019}
\DIFdel{Muller JZ.
}%DIFDELCMD < \newblock %%%
...
%DIFDELCMD < \bibitem{siler_who_2022}
\DIFdel{Siler K, Larivi}%DIFDELCMD < {%%%
\DIFdel{\`e}%DIFDELCMD < }%%%
\DIFdel{re V.
}%DIFDELCMD < \newblock %%%
\DIFdel{Who Games Metrics and Rankings? }%DIFDELCMD < {{%%%
\DIFdel{Institutional}%DIFDELCMD < }} %%%
...
%DIFDELCMD < %%%
\DIFdelend \bibitem{bihari_review_2023}
...
\DIFaddbegin \bibitem{mennicken_what_2019}
\DIFadd{Mennicken A, Espeland WN.
}\newblock \DIFadd{What's New with Numbers? }{{\DIFadd{Sociological}}} \DIFadd{Approaches to the Study of Quantification.
}\newblock \DIFadd{Annual Review of Sociology. 2019;45:223--245.
}

\bibitem{muller_tyranny_2019}
\DIFadd{Muller JZ.
}\newblock \DIFadd{The Tyranny of Metrics.
}\newblock \DIFadd{Princeton University Press; 2019.
}

\DIFaddend \bibitem{berg_producing_2016}
...
\DIFadd{Edwards MA, Roy S.
}\newblock \DIFadd{Academic }{{\DIFadd{Research}}} \DIFadd{in the 21st }{{\DIFadd{Century}}}\DIFadd{: }{{\DIFadd{Maintaining Scientific Integrity}}} \DIFadd{in a }{{\DIFadd{Climate}}} \DIFadd{of }{{\DIFadd{Perverse Incentives}}} \DIFadd{and }{{\DIFadd{Hypercompetition}}}\DIFadd{.
}\newblock \DIFadd{Environmental Engineering Science. 2017;34(1):51--61.
}\newblock \DIFadd{doi:}{\DIFadd{10.1089/ees.2016.0223}}\DIFadd{.
}

\bibitem{siler_who_2022}
\DIFadd{Siler K, Larivi}{\DIFadd{\`e}}\DIFadd{re V.
}\newblock \DIFadd{Who Games Metrics and Rankings? }{{\DIFadd{Institutional}}} \DIFadd{Niches and Journal Impact Factor Inflation.
}\newblock \DIFadd{Research Policy. 2022;51(10):104608.
}\DIFaddend 
