...
%DIF ADD hirsch_plosone_randr1.tex              Thu Apr 18 16:23:51 2024
...
\textbf\newline{Inequality in measuring scholarly success: Variation in
the \emph{h}-index within and between
disciplines} %DIF >  Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\DIFdelbegin %DIFDELCMD < \newline
...
penalties within fields}\DIFaddend . Conclusions include continued caution when
...
differences in scholar's }\emph{\DIFdel{h}}%DIFAUXCMD
\DIFdel{-index, or Hirsch index, we use data on over 50,000 high-performing scientists across 174 disciplines in the
...
models predicting }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bihari_review_2023}}\hskip0pt%DIFAUXCMD
. This study extends
prior work on metrics in science generally and }\DIFaddend the \emph{h}-index
\DIFdelbegin \DIFdel{with a range of factors including
...
distinct within and between disciplinary effects on }\DIFdelend \DIFaddbegin \DIFadd{specifically by conceptualizing the inequality process as occurring via
intra-disciplinary and inter-disciplinary dynamics. Variation in the
}\DIFaddend \emph{h}-index \DIFdelbegin \DIFdel{scores, consistent with calls to develop multilevel analyses of gender
...
}\DIFdelend \DIFaddbegin \DIFadd{may result from ``apples-to-oranges'' comparisons across
disciplines and fields due to specific factors, like gender }\DIFaddend and sole
authorship\DIFdelbegin \DIFdel{penalties. Results further show substantial between
discipline differences related to sole authorship. Moreover, roughly a
third of the variation in }\emph{\DIFdel{h}}%DIFAUXCMD
\DIFdel{-index is between rather than }\DIFdelend \DIFaddbegin \DIFadd{, but also occur }\DIFaddend within disciplines, \DIFdelbegin \DIFdel{suggesting an important apples-to-oranges problem when
comparing the }\emph{\DIFdel{h}}%DIFAUXCMD
\DIFdel{-index across disciplines. This apples-to-oranges
problem results in significant differences across fields as well,
especially in terms of }\DIFdelend \DIFaddbegin \DIFadd{as scholars experience
differing publishing trajectories dependent on gender and }\DIFaddend sole
authorship.
\DIFdelbegin \DIFdel{Conclusions highlight how the
...
}\DIFdelend \DIFaddbegin \DIFadd{the relationship of }\DIFaddend the \emph{h}-index \DIFaddbegin \DIFadd{to gender and sole
authorship at both the individual and disciplinary level}\DIFaddend .
\end{quote}
...
\DIFaddendFL \caption{\label{tbl-partition}\DIFdelbeginFL \DIFdelFL{Parition }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Partition }\DIFaddendFL of variance in \emph{h}\DIFdelbeginFL \DIFdelFL{-indexscores }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-index
scores }\DIFaddendFL between and within disciplines}
\DIFdelbeginFL %DIFDELCMD < \tabularnewline
%DIFDELCMD < %%%
\DIFdelendFL 
...
disciplines in terms of sole authorship , supporting }\DIFdelend \DIFaddbegin \DIFadd{Disciplines
with more overall sole authorship have substantially lower mean
}\emph{\DIFadd{h}}\DIFadd{-index scores overall, consistent with }\DIFaddend the teamwork variation
...
reverses when including sole
authorshipand
that }\DIFdelend \DIFaddbegin \DIFadd{H3a)
